...
public: static void Mass() // Создаём функцию при вызове которой каждый раз будет создаваться новый массив чисел
{
    ...
    const int ROW = 10; // Объявляем константную переменную хранящую число строк массива
    const int COL = 15; // Объявляем константную переменную хранящую число столбцов массива
...
int main()
{
    ...
    while (1) // Создаём цикл While для того, чтобы наша программа не закрылась после того как отработает 1 раз
    {
        ...
        {                        //сгенерирует нашу матрицу(массив) и выведет её на экран
            ...
            Sleep(1000);         // Устанавливаем задержку между выводом нового массива на экран
            cout << endl;        // Делаем отступ между массивами
        }
    }
}